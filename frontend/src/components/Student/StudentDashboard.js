...
function StudentDashboard() {
  ...
  // const courseId = course._id; // from props or location.state

  const getProgress = async () => {
    try {
      const res = axios.get("http://localhost:5000/progress/:userId/:courseId"); // changing later
      if (res?.status === "Completed") {
        setProgress(100);
        setIsCompleted(true);
      } else {
        setProgress(50); // or whatever logic fits your app
        setIsCompleted(false);
      }
    } catch (error) {
      console.error("Error in getting progress bar", error);
    }
  };
  ...
  const courses = [
    ...
    },
    {
      ...
      status: "Pending",
      content:
        "imply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's ",
      img: { CourseImg },
    },
    {
      id: 3,
                ...
                return (
                  ...
                  >
                    <div className="relative">
                      <span
                        class={`inline-flex absolute z-10 top-2 right-2 items-center ${
